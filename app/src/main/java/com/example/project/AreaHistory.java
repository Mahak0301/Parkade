...
public class AreaHistory extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(!utils.isNetworkAvailable(getApplication())){
            Toast.makeText(AreaHistory.this, "No Network Available!", Toast.LENGTH_SHORT).show();
        }
    }
                ...
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        for (DataSnapshot dataSnapshot : snapshot.getChildren()) {
                            db.getReference().child("BookedSlots").orderByChild("placeName").equalTo(dataSnapshot.getKey())
                                    .addListenerForSingleValueEvent(new ValueEventListener() {
                                        @Override
                                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                                            for (DataSnapshot dataSnapshot : snapshot.getChildren()) {
                                                BookedSlots bookedSlot = dataSnapshot.getValue(BookedSlots.class);
                                                BookedSlotKey bookedSlotKey=new BookedSlotKey(bookedSlot,dataSnapshot.getKey(),bookedSlot.numberPlate);
